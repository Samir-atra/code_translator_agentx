...
```
This command will:
- Start the agent servers using the commands specified in translator_scenario.toml
...
To run this example manually, start the agent servers in separate terminals, and then in another terminal run the A2A client on the translator_scenario.toml file to initiate the assessment.
...
scenarios/
└─ code_translator/                     # implementation of the code translator example
   ├─ adk_translator_judge.py      # alternative green agent impl using Google ADK
   ├─ translator_judge_common.py   # models and utils shared by above impls
   ├─ translator.py               # translator agent (Google ADK)
   └─ translator_scenario.toml            # config for the code translator example
```
